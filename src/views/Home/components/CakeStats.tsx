...
const CakeStats = () => {
  ...
  // const totalSupply = new BigNumber(286306483990147782957)
  const givePrice = usePriceCakeBusd()
  const tokenAddress = getGiveAddress()
    ...
    <StyledCakeStats>
      <CardBody>
        ...
        <Row>
          ...
          {cakeSupply && <CardValue fontSize="14px" value={1280000} decimals={0} />}
        </Row>
        <Row>
          <Text fontSize="14px">{t('New GIVE/block')}</Text>
          {cakeSupply && <CardValue fontSize="14px" decimals={0} value={givePerBlock} />}
        </Row>
      </CardBody>
    </StyledCakeStats>
  )
