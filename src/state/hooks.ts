...
export const useFarmUser = (pid) => {
  ...
  return {
    ...
    tokenBalance: farm.userData ? farm.pid === 3 ? new BigNumber(parseFloat(farm.userData.tokenBalance) / 10**12) : new BigNumber(farm.userData.tokenBalance) : BIG_ZERO,
...
export const useTotalValue = (): number => {
  ...
  for (let i = 0; i < farms.length; i++) {
    ...
    if (
      !farms[i].isSingleToken &&
      farms[i].multiplier !== '0X' &&
      !farms[i].isHiddenFarm &&
      farms[i].lpTotalInQuoteToken &&
      farms[i].quoteToken.busdPrice
    ) {
...
// for vault total TVL
export const useTotalValueVault = (): number => {
  const { data: farms } = useVaults()
  ...
  return totalLiquidity <= 6000 ? 0 : totalLiquidity
}
