...
export const vaultStake = async (vaultChefContract, pid, amount, account, tokenDecimals) => {
  return vaultChefContract.methods
    .deposit(pid, new BigNumber(amount).times(tokenDecimals).toString())
    .send({ from: account, gas: DEFAULT_GAS_LIMIT })
    .on('transactionHash', (tx) => {
      return tx.transactionHash
    })
...
export const vaultUnstake = async (vaultChefContract, pid, amount, account, tokenDecimals) => {
  return vaultChefContract.methods
    .withdraw(pid, new BigNumber(amount).times(tokenDecimals).toString())
    .send({ from: account, gas: DEFAULT_GAS_LIMIT })
    .on('transactionHash', (tx) => {
...
export const vaultUnstakeAll = async (vaultChefContract, pid, amount, account, tokenDecimals) => {
  return vaultChefContract.methods
    .withdrawAll(pid)
    .send({ from: account, gas: DEFAULT_GAS_LIMIT })
    .on('transactionHash', (tx) => {
...
const WBNB_TOKEN = new Token(chainId, tokens.wmetis.address[process.env.REACT_APP_CHAIN_ID], 18) // CHANGED TO WETH, TODO: remove hardcoded chainID
