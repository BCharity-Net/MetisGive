...
import { stake, sousStake, sousStakeBnb, vaultStake, getReferrerAddress } from 'utils/callHelpers'
...
export const useVaultStake = (pid: number, tokenDecimals: BigNumber, willUseMVaultChef: boolean) => {
  const { account } = useWeb3React()
  ...
  const handleStake = useCallback(
    async (amount: string) => {
      const txHash = await vaultStake(contractToUse, Math.floor(pid), amount, account, tokenDecimals)
      console.info(txHash)
    },
