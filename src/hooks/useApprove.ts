...
// Approve a vault
export const useVaultApprove = (lpContract: Contract, willUseMVaultChef: boolean) => {
  const { account } = useWeb3React()
  ...
  const handleApprove = useCallback(async () => {
    try {
      const tx = await approve(lpContract, contractToUse, account)
      return tx
    } catch (e) {
      return false
    }
  }, [account, lpContract, contractToUse])
...
export const useCakeVaultApprove = (setLastUpdated: () => void) => {
  const { account } = useWeb3React()
  const [requestedApproval, setRequestedApproval] = useState(false)
  const { t } = useTranslation()
  const { toastSuccess, toastError } = useToast()
